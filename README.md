...
### 주요 개선사항 (v2.1)
- ✅ 세션 기반 로그인 상태 유지 (새로고침해도 로그인 유지)
- ✅ 사용자/담당자 분리 관리 (성능 최적화)
- ✅ 담당자 추가 시 실시간 화면 업데이트 (리로드 없음)
- ✅ 더 빠른 응답 속도
...
| 컬럼명 | 설명 | 예시 |
|--------|------|------|
| 사용자ID | 이메일 주소 (로그인ID) | admin@company.com |
| 이메일 | 로그인용 이메일 | admin@company.com |
| 이름 | 사용자 이름 | 김관리자 |
| 비밀번호 | 로그인 비밀번호 | admin123 |
| 역할 | 관리 역할 | 팀장/관리자/최종관리자 |
...
| 컬럼명 | 설명 | 예시 |
|--------|------|------|
| 담당자ID | 담당자 고유 식별자 | MEM001 |
| 이름 | 담당자 이름 | 박담당 |
| 역할 | 담당 역할 | PD/편집자/작가 등 |
| 소속팀ID | 소속 팀 | T01 |
| 활성상태 | 활성화 상태 | Y/N |
| 생성일시 | 생성 시간 | 2025/01/20 10:00:00 |
...
### 세션이 유지되지 않는 경우
1. 브라우저의 쿠키/캐시 설정 확인
2. Apps Script 프로젝트 설정에서 권한 확인
3. PropertiesService 사용 권한 확인

### 담당자 추가가 느린 경우
1. v2.1에서는 화면 업데이트가 즉시 되도록 개선됨
2. 여전히 느리다면 네트워크 연결 확인
3. 구글 서버 상태 확인
...
### Apps Script 편집기에서 실행 가능한 테스트 함수들

#### 1. 연결 테스트
```javascript
testConnection()  // 스프레드시트 연결 및 데이터 수 확인
```

#### 2. 세션 테스트
```javascript
testSession()     // 현재 저장된 세션 확인
clearAllSessions() // 모든 세션 초기화 (로그아웃 효과)
```

#### 3. 샘플 데이터 생성
```javascript
createSampleData() // 테스트용 사용자, 담당자, 채널 데이터 생성
```

#### 4. 초기화
```javascript
initSheets()      // 모든 시트 구조 생성/초기화
```

### 브라우저 콘솔에서 사용 가능한 디버그 명령

```javascript
// 현재 상태 확인
console.log('현재 사용자:', currentUser);
console.log('현재 역할:', currentRole);
console.log('앱 데이터:', appData);
